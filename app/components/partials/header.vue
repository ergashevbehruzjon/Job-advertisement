<template>
  <header class="flex items-center py-4 md:py-8">
    <div class="header__logo" style="margin-left:auto;margin-right:auto;display: flex;justify-content: center;">
      <nuxt-link to="/">
        <img :src="logo" style="width:50px;display:inline;" alt="Logo" />
        <h1 style="display:inline;font-size:2em;font-weight: bolder;vertical-align:middle;">Head teacher for Presidential School in Jizzakh</h1>
      </nuxt-link>
    </div>

    <!-- <nav class="nav ml-auto">
      <ul class="flex flex-row items-center sm:mt-4 sm:pt-4 md:mt-0 md:pt-0 md:mr-4 lg:mr-8">
        <li>
          <nuxt-link to="/blog" class="block font-medium px-4 py-1 md:p-2 lg:px-4">
            Blog
          </nuxt-link>
        </li>

        <li
          v-for="(page, index) in pages"
          :key="index"
          class="block font-medium px-4 py-1 md:p-2 lg:px-4"
        >
          <nuxt-link :to="`/${page.slug}`">{{ page.title }}</nuxt-link>
        </li>
      </ul>
    </nav> -->
  </header>
</template>

<script lang="ts">
import { Component, Vue } from 'nuxt-property-decorator';
import settings from '@/content/settings/general.json';

@Component
export default class Header extends Vue {
  get pages(): Page[] {
    return this.$store.state.pages;
  }

  logo = settings.logo;
}
</script>

<style lang="scss">
.nav {
  ul li > a {
    &.nuxt-link-active {
      color: $bluise;
      @apply font-bold;
    }
  }
}
</style>
